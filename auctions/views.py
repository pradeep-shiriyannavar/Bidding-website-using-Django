...
from .models import User, Listing, Bid, Comment, Category,EndTime
...
def index(request):
    ...
    return render(request, "auctions/index.html", {
        ...
        #lastb=(Bid.objects.last()).bidder
        'winner': Bid.objects.last().bidder,
...
class NewListingForm(forms.Form):
    ...
    endtime= forms.DateTimeField(label="End Date",widget=forms.DateTimeInput(format=('%Y-%m-%dT%H:%M'), attrs={'type': 'datetime-local'}))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new = Listing(title=title, description=description, current_bid=bid, image_url=url,
                          category=Category.objects.get(id=category), user=request.user,endtime=endtime)
